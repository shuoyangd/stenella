...
def main(options):
  ...
  for word_text_line, subword_text_line, word_tag_line, subword_tag_line in \
    ...
    for word_idx, word_tok in enumerate(word_toks):
      ...
      # conditioned on the word tag, we are going to do different things
      word_tag = word_tags[token_tag_idx(word_idx)]

      # if the word is OK, all subwords should be OK
      if word_tag == "OK":
        new_subword_tags.extend(["OK"] * (subword_tag_span_end - subword_tag_span_bgn + 1))
      # else, copy the corresponding span of bpe-tags over,
      # but if every bpe-tags in the corresponding span is "OK", set them all to "BAD" to
      # optimize for recovery rate (this should occur less than 3%)
      else:
        corresp_tags = subword_tags[subword_tag_span_bgn:subword_tag_span_end+1]
        if corresp_tags == ["OK"] * len(corresp_tags):
          corresp_tags = ["BAD"] * len(corresp_tags)
        new_subword_tags.extend(corresp_tags)
