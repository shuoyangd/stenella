...
def compute_scores(true_tags, test_tags):
    ...
    p_all_scores = precision_score(flat_true, flat_pred, average=None, pos_label=None)
    r_all_scores = recall_score(flat_true, flat_pred, average=None, pos_label=None)
    ...
    ret = np.append(f1_all_scores, mcc)
    ret = np.append(ret, p_all_scores)
    ret = np.append(ret, r_all_scores)
    return ret
...
def make_breakdown_files(ref_mt_tags, sub_mt_tags):
    ref_mt_dir = os.path.dirname(ref_mt_tags)
    sub_mt_dir = os.path.dirname(sub_mt_tags)

    ref_word_tags_file = open(os.path.join(ref_mt_dir, "breakdown_words.txt"), 'w')
    ref_gap_tags_file = open(os.path.join(ref_mt_dir, "breakdown_gaps.txt"), 'w')
    for line in open(ref_mt_tags):
      tags = line.strip().split()
      out_word_line = ' '.join(tags[1::2]) + "\n"
      out_gap_line = ' '.join(tags[0::2]) + "\n"
      ref_word_tags_file.write(out_word_line)
      ref_gap_tags_file.write(out_gap_line)
    ref_word_tags_file.close()
    ref_gap_tags_file.close()

    sub_word_tags_file = open(os.path.join(sub_mt_dir, "breakdown_words.txt"), 'w')
    sub_gap_tags_file = open(os.path.join(sub_mt_dir, "breakdown_gaps.txt"), 'w')
    for line in open(sub_mt_tags):
      tags = line.strip().split()
      out_word_line = ' '.join(tags[1::2]) + "\n"
      out_gap_line = ' '.join(tags[0::2]) + "\n"
      sub_word_tags_file.write(out_word_line)
      sub_gap_tags_file.write(out_gap_line)
    sub_word_tags_file.close()
    sub_gap_tags_file.close()
...
def main(ref_dir: 'dir containing all ref files', submit_dir: 'dir containing all predicitiosn files'):
    ...
    make_breakdown_files(ref_mt_tags, submit_mt_tags)

    ref_word_tags = os.path.join(ref_dir, "breakdown_words.txt")
    sub_word_tags = os.path.join(submit_dir, "breakdown_words.txt")

    f1_bad_tg, f1_good_tg, mcc_tg, p_bad_tg, p_good_tg, r_bad_tg, r_good_tg = evaluate(None, ref_word_tags, sub_word_tags)
    ...
    ref_gap_tags = os.path.join(ref_dir, "breakdown_gaps.txt")
    sub_gap_tags = os.path.join(submit_dir, "breakdown_gaps.txt")

    f1_bad_gap, f1_good_gap, mcc_gap, p_bad_gap, p_good_gap, r_bad_gap, r_good_gap = evaluate(None, ref_gap_tags, sub_gap_tags)
    ...
    print('mt_p-bad: {:.4}'.format(p_bad_tg))
    print('mt_p-good: {:.4}'.format(p_good_tg))
    print('mt_r-bad: {:.4}'.format(r_bad_tg))
    print('mt_r-good: {:.4}'.format(r_good_tg))
    ...
    print("gaps_p-bad: {:.4}".format(p_bad_gap))
    print('gaps_p-good: {:.4}'.format(p_good_gap))
    print("gaps_r-bad: {:.4}".format(r_bad_gap))
    print('gaps_r-good: {:.4}'.format(r_good_gap))
